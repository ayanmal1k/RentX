...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      const decoded = bs58.decode(userAddress);
      if (decoded.length < 32 || decoded.length > 44) {
        return NextResponse.json(
          { error: 'Invalid Solana wallet address. Please reconnect your wallet.' },
          { status: 400 }
        );
      }
    } catch {
    ...
    const xoraAmount = usdValue / presalePriceUsd;
    ...
    if (treasuryPublicKey.toBase58() !== TREASURY_WALLET_ADDRESS) {
      return NextResponse.json(
        { error: 'Treasury secret key does not match the configured treasury wallet address.' },
        { status: 500 }
      );
    }
    ...
    const amountInBaseUnits = BigInt(Math.max(1, Math.round(xoraAmount * 10 ** SOLANA_TOKEN_DECIMALS)));
    ...
    return NextResponse.json({
      ...
      payoutTxHash,
      presalePriceUsd,
      message: `Successfully sent ${xoraAmount.toFixed(9)} GTTWRLD tokens to ${userAddress.slice(0, 6)}...${userAddress.slice(-4)}`,
    });
  } catch (error: any) {
